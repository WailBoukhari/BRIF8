...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['category_image']) && $_FILES['category_image']['error'] === UPLOAD_ERR_OK) {
        // Use the ImageUploader class to handle image upload
        $imagePath = ImageUploader::uploadImage('category_image');

        // Check if the upload was successful
        if ($imagePath) {
            // You can do something with the $imagePath if needed
            // For example, update the category with the new image path
    ...
    $category = new Category($categoryId, $categoryName, $otherFormFieldValue1, $isDisabled);
